...
namespace BanManager
{
    ...
    {
        private long qqid;
        private string reason, qqss, evidence;
        /// <summary>
        /// 初始化一个被ban的人
        /// </summary>
        /// <param name="qqid">QQ号（防止改名字）</param>
        /// <param name="reason">封禁原因</param>
        /// <param name="qqss">QQ号截图路径</param>
        /// <param name="evidence">证据（文件路径）</param>
        public BanPerson(long qqid, string reason, string qqss, string evidence)
        {
            this.qqid = qqid;
            this.reason = reason;
            this.qqss = qqss;
        ...
        {
            return qqid.ToString() + "*" + reason + "*" + qqss + "*" + evidence;
        }
        public static BanPerson parse(string data)
        {
            ...
            return new BanPerson(id, s[1], s[2], s[3]);
        }
    }
}
